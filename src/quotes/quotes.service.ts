...
export class QuotesService {
    ...
    async getWeeklyQuotation(setNo: number, projectObj: any, searchQuery: string, week: number) {
        try {
            let limit = 10;
            ...
            //      aggregate([, {$match: {requestWeek: 22}}]);
            ...
            const [result, totalCount] = await Promise.all([
                ...
                requestCollection
                    .aggregate([
                        { $addFields: { requestWeek: { $isoWeek: '$requestTime' } } }, 
                        {
                            $match: {
                                requestWeek : week,
                                $or: [
