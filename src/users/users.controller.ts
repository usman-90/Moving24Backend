...
export class UsersController {
  ...
  quoteRequest(@Body() body: Record<string, any>, @Req() req: any) {
    let temp: any = {
      ...
      minBudgetRange: body?.budgetRange?.minimum
        ? parseInt(body.budgetRange.minimum)
        : body?.budgetRange?.minimum,
      maxBudgetRange: body?.budgetRange?.maximum
        ? parseInt(body.budgetRange.maximum)
        : body?.budgetRange?.maximum,
  ...
  async getAdminDetails(@Body() body: Record<string, any>, @Req() req: any) {
    ...
    const res = await this.adminService.getAdminDetails(
      req.query.id,
      projectionObj,
    );
    return res;
